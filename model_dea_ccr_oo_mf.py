...
def solve_dea(target_dmu, p_inputs, p_outputs):
    ...
    v_wout[r] = model.addVars(s_output, lb=0.00001)
    
    # Model constraints
    for d in s_dmu:
        model.addConstr(quicksum(p_outputs[d][r] * v_wout[r] for r in s_output) <=
                        quicksum(p_inputs[d][i] * v_win[i] for i in s_input))
    
    model.addConstr(quicksum(p_outputs[p_target_dmu][i] * v_wout[i] for i in s_output) <= 1)
    model.addConstr(quicksum(p_outputs[p_target_dmu][i] * v_wout[i] for i in s_output) >= 0.99999)
    
    # Model objective
    model.setObjective(quicksum(p_inputs[p_target_dmu][r] * v_win[r] for r in s_input), GRB.MINIMIZE)
